...
def convert_bitmap(bitmap_handle, q):
    ...
    write_texture(tex, f"{save_direc}/{q}.png")
    
# DELETE UNDER BEFORE DEF WRITE TEXTURE
    d = {x.split('_bitmap_resource_handle_')[-1][-2]: x for x in os.listdir(folder) if "_normal" in x and q in x}
    if len(d.keys()) == 0:
        print(f"{bitmap_handle} has no texture files to use, skipping...")
        return
    chunk_to_use = d[max(d.keys())]
